...
class _AddInventoryState extends State<AddInventory> {
  ...
  Future<void> _submitInventory() async {
    ...
    // Save the entire list of products for the current date
    await _database.create(currentDate, inventory);
...
class Database {
  ...
  // Method to add inventory data grouped by date to Firestore
  Future<void> create(String date, List<Map<String, dynamic>> inventory) async {
    try {
      await _fire.collection("inventories").doc(date).set({
        "date": date,
        "items": inventory,
      });
    } catch (e) {
